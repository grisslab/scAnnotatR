...
setMethod("classify_cells", c("classify_obj" = "Seurat"), 
  ...
  if (any(pred_cells != "")) {
    ...
    if (ignore_ambiguous_result == FALSE) 
      classify_obj[['most_probable_cell_type']] <- simplify_prediction(classify_obj[[]], mat, classifiers)
  }
...
setMethod("classify_cells", c("classify_obj" = "SingleCellExperiment"), 
  ...
  if (any(pred_cells != "")) {
    ...
    if (ignore_ambiguous_result == FALSE) 
      classify_obj$most_probable_cell_type <- simplify_prediction(SummarizedExperiment::colData(classify_obj), 
                                                                  mat, classifiers)
  }
  return(classify_obj)
})
