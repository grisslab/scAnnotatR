...
subset_models <- function(model_list, model_names) {
  ...
  if (length(model_names) > length(model_list)) {
    stop("Error: The number of selected classifiers is larger ",
         "than the available ones.", call. = FALSE)
  }

  # make sure all classifiers exist
  if (any(!model_names %in% names(model_list))) {
    stop("Error: The following selected classifiers could not ",
         "be found: ", paste0(
          model_names[!model_names %in% names(model_list)],
          collapse = ", "))
  }

  # get a list of all required parent models
  parent_models <- c()

  for (model_name in model_names) {
    parent_model <- parent(model_list[[model_name]])

    if (!is.na(parent_model)) {
      parent_models <- c(parent_models, parent_model)
    }
  }

  # if no parent models are required, simply return the subset
  if (length(parent_models) < 1 || all(parent_models %in% model_names)) {
    return(model_list[model_names])
  }

  # get the missing parent models
  missing_parents <- parent_models[!parent_models %in% model_names]

  # ensure that they are available
  if (any(!missing_parents %in% names(model_list))) {
    stop("Error: The following required parent classifiers could not ",
         "be found: ", paste0(
          missing_parents[!missing_parents %in% names(model_list)],
          collapse = ", "
         ))
  }

  # add them
  message("Including required parent classifiers: ",
          paste0(missing_parents, collapse = ", "))

  complete_names <- c(model_names, missing_parents)

  return(model_list[complete_names])
}
